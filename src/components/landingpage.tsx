"use client"
import  { legalNeeds } from "@/lib/utils/legalneeds";
...
export default function LandingPage() {
  ...
  const filteredNeeds = legalNeeds.filter((item) =>
      ...
      <section className="max-w-6xl mx-auto px-6 py-12">
        <div className="w-full md:w-2/3 max-w-2xl mx-auto">
          ...
          <p className="text-slate-500 mb-6">
            Verified advocates and legal experts all over world
          </p>

          {/* 🔍 SEARCH BAR */}
          <div className="flex bg-white shadow rounded-lg relative">
            {/* Your Needs Autocomplete */}
            <div className="flex-1 relative">
              <input
                className="w-full px-4 py-3 outline-none"
                placeholder="Your Needs (Divorce, Bail, Property...)"
                value={query}
                onChange={(e) => {
              ...
              {showSuggestions && query && (
                <ul className="absolute z-20 w-full bg-white border border-gray-200 rounded-md mt-1 max-h-60 overflow-y-auto shadow">
            ...
            {/* Area */}
            <select
              className="px-4 py-3 border-l border-gray-100"
              value={selectedCity ?? ""}
            ...
            <Link
              className="bg-teal-600 text-white px-6 py-3 flex items-center justify-center"
              href={`/search/${selectedNeed?.value}/${selectedCity}`}
            >
              Get Connected
            </Link>
          </div>
          ...
          <div className="flex gap-4 mt-6">
            <div className="bg-white rounded-xl shadow p-5 text-center flex-1">
              ...
              <p className="text-sm text-slate-500">
                Get help from verified lawyers for civil and property matters.
              </p>
            </div>
            <div className="bg-white rounded-xl shadow p-5 text-center flex-1">
              <div className="text-2xl mb-3">⚖️</div>
              ...
              <p className="text-sm text-slate-500">
                Experienced litigators for civil and criminal matters.
              </p>
            </div>
            <div className="bg-white rounded-xl shadow p-5 text-center flex-1">
              <div className="text-2xl mb-3">🏛️</div>
              ...
              <p className="text-sm text-slate-500">
                Advice and representation for family and property disputes.
              </p>
            </div>
          </div>
        </div>
